...
function createContaktButton() {
  ...
  $buttonAdd.addEventListener('click', function (event) {
    ...
    if (contactsNumber.length >= 9) {
      $buttonAdd.classList.add('noneAdd');
    }
...
function createContaktInput() {
  ...
  $contactVar.addEventListener('change', function (event) {
    event.preventDefault();
    $contactInput.value = "";
    if ($label.lastChild.classList.contains('contact-del-btn')) {
      $label.lastChild.remove();
    }

    if ($label.classList.contains('delYes')) {
      $label.classList.remove('delYes');
    }
  ...
  $contactInput.addEventListener('input', function () {
    if ((document.querySelector('.contact-input').value !== '') && (this.parentNode.classList.contains('delYes') === false)) {
  ...
  $contactDelBtn.addEventListener('click', function (event) {
    event.preventDefault();
    $contactLi.remove();
    let contactsNumb = document.querySelectorAll('.contact-item');
    if (contactsNumb.length <= 9) {
      document.getElementById('add').classList.remove('noneAdd');
    }
  });
...
function createTr(client) {
  ...
  function editClient(objClient) {
    ...
    if (objClient.contacts !== null) {
      ...
      if (contaktArray.length > 9) {
        $butAddEdite.children[1].classList.add('noneAdd')
      }
      contaktArray.forEach(contactValue => {
        const $liEdit = createContaktInput();
        const editLabel = $liEdit.children[0];
        editLabel.classList.add('delYes');
        ...
        $contactDelEdit.addEventListener('click', function (event) {
          event.preventDefault();
          $liEdit.remove();
          let contactsLiNumberDel = document.querySelectorAll('.contact-item');
          if (contactsLiNumberDel.length <= 9 && !null) {
            $add.classList.remove('noneAdd');
          }
        });
...
function createModalScreen() {
  ...
  $modalScreen.classList.add("modal__screen", "open");
  ...
  if (val !== "") {
    return arr.filter(el => {
      if (key === "search") {
        ...
        return String(searchRezultLower).includes(val.toLowerCase());
      }
      else {
        return String(el[key]).includes(val.toLowerCase())
      }
    })
